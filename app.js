...
(() => {
  ...
  function renderTimeline() {
    ...
    $('#tab-timeline').innerHTML = `
      ...
      ${timelineShell('Assignments', 'projectTimeline', projectTimelineLabels(), projectTimelineRows(min, months, width), header, width, min, max, true)}
  ...
  function timelineShell(title, id, labels, rows, header, width, min, max, showPersonPalette = false) {
    const marker = currentMonthMarker(min, max);
    const paletteHtml = showPersonPalette ? `<div class="person-palette">${visiblePersons().map(p => `<span class="person-chip" draggable="true" data-drag-person="${p.id}" style="border-color:${colorFor(p.id)}">${esc(personName(p))}</span>`).join('')}</div>` : '';
    return `<div class="timeline-shell"><div class="timeline-title">${title}</div><div class="timeline-body">
      <div class="timeline-labels">${paletteHtml}<div class="timeline-label-spacer"></div>${labels}</div>
